...
IPHONE_IP = "10.3.0.63"  # IP iPhone cho phép
...
def handle_mdns_query(pkt):
    if pkt.haslayer(IP) and pkt.haslayer(UDP) and pkt.haslayer(DNS):
        ...
        # Lưu IP và MAC của iPhone
        iphone_info["ip"] = src_ip
        iphone_info["mac"] = src_mac
...
def handle_mdns_response(pkt):
    if pkt.haslayer(IP) and pkt.haslayer(UDP) and pkt.haslayer(DNS):
        ...
        if src_ip != CHROMECAST_IP:
            return

        # Kiểm tra xem đã lưu thông tin iPhone chưa
        if not iphone_info["ip"] or not iphone_info["mac"]:
            logging.debug(f"Chưa có thông tin iPhone, bỏ qua Response từ {CHROMECAST_IP}")
            return
